...
describe('x', function() {
  ...
  const run = async (url, prompt) => {
    const l = new Learner();
    const kb = await l.learn(
      { url, prompt },
      (partial) => {
        console.log('== Data so far ==');
        console.log(JSON.stringify(partial, null, 2));
        console.log('');
        printKnowledgeBase(partial);
      });
  ...
  it('should learn https://www.intel.com/content/www/us/en/products/overview.html', async () => {
    await run(
      'https://www.intel.com/content/www/us/en/products/overview.html',
      'scrape desktop and laptop CPUs and GPUs info and specs',
    );
  });

  it('should learn https://home.co.uk/', async () => {
    await run(
