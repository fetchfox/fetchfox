...
export const Document = class {
  ...
  async read(resp, reqUrl, reqOptions, options) {
    ...
    logger.debug(
      `${this} Done reading body for ${this.url}, took ${tookRead / 1000} sec and got ${this.body.length} bytes`,
    );
    ...
    logger.info(
      `${this} Done loading for ${this.url}, took total of ${took / 1000} sec, got ${this.body.length} bytes`,
    );
  }
    ...
    for (const el of els) {
      ...
      for (const a of as) {
        ...
        try {
          ...
          logger.warn(
            `${this} Skipping invalid link: ${this.url} href=${href} url=${this.url} html=${html.substr(0, 40)}: ${e}`,
          );
          continue;
        }
