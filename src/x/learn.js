...
export class Learner {
  ...
  async analyzeLinksTo({ docs, prompt }) {
    const urls = [];
    const min = new TagRemovingMinimizer({ cache: this.cache });
    const seen = {};
    for (const doc of docs) {
  ...
  async analyzeItems({ docs, prompt }) {
    const urls = joinDocsUrl(docs);
    const htmls = joinDocsHtml(docs);
    const context = { urls, prompt, htmls };
    const { prompt: itemsPrompt } = await prompts
      .availableItems
      .renderCapped(context, 'htmls', this.ai);

    // console.log('analyzeItems prompt', itemsPrompt);
    const answer = await this.ai.ask(itemsPrompt, { format: 'jsonl' });
    // console.log('analyze items answer.partial:', JSON.stringify(answer.partial));
    // console.log('analyze items answer.partial was for urls:', urls);
