...
describe("pokemondb.net", function () {
    ...
    const json = {
      steps: [
        ...
        },
        {
          ...
          args: {
            query:
              "Look for links to individual Pokemon. Ignore links to advertisements and navigation links. Only Pokemon characters, no other pages.",
            limit: "10",
          },
        },
        {
  ...
  it("should terminate with limit", async function () {
    ...
    const f = await fox
      .config({
        fetcher: [
          "playwright",
          { headless: true, interval: 1000, intervalCap: 1 },
        ],
      })
      .init("https://pokemondb.net/pokedex/national")
      .crawl({
  ...
  it("should filter for fire type pokemon", async () => {
        ...
        },
        {
          ...
          args: {
            questions: {
              ...
              types:
                "What are the types of this Pokémon? Format: Type1 · Type2",
    ...
    const fireTypes = out.items.filter(
      (i) => i.types.toLowerCase().indexOf("fire") != -1,
    );
