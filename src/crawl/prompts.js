...
export const gather = new Template(
  ['query', 'links'],
  `You are part of a web crawling program, and your goal is to pick out relevant links in a list. The list contains the inner text of links, and also their URLs. You will take this list, look for links that match the user prompt, and generate one or more regex that match only the items matching the query.

Your response will be ONLY JSONL with "regex" field that matches urls. The "regex" field will be used to generate the results later, you only need to include the "regex" field.  Remember to format as JSONL and give a regex string suitable for use in javascript.  Each regex will be searched for against only urls, not html or anchor tags.  Do not include flags.

Follow these important rules:
- The entire array should be JSONL, with a single object per regex
...
- Generally avoid matching links with no link text.
...
Create regex matching links matching the user query: {{query}}
