...
import { transform, TagsTransformer, TextOnlyTransformer } from '../transform/index.js';
...
export const DirectExtractor = class extends BaseExtractor {
  ...
  async *_run(doc, questions, options) {
    ...
    const transformers = [new TagsTransformer()]

    switch (options?.view) {
      case 'text':
        transformers.push(new TextOnlyTransformer());
        break;

      case 'links':
        transformers.push(new LinksTransformer());
        break;
    }

    const out = await transform(doc.html, transformers);
    const body = out.html;
    ...
    // console.log('scrapePrompts[0]', scrapePrompts[0]);
...
// { "_meta": true, "analysis": "...your analysis here..."}`;
    ...
    case 'auto':
      return  `The user has not specficied whether they want multiple items or a single item as a result. Use your judgement to figure this out.`;
