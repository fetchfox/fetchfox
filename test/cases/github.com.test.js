...
describe("github.com", function () {
  ...
  it("should do basic scrape @run @fast", async () => {
    ...
    const wf = await fox
      ...
      .crawl({
        query:
          "find urls of commits, format: https://github.com/bitcoin/bitcoin/commit/...",
        limit: 5,
      })
      .extract({
        questions: {
  ...
  it("should do complex scrape @disabled", async () => {
    ...
    const out = await fox
      ...
      .crawl({
        query:
          "get URL of the author github profile. MUST match pattern: https://github.com/[username]",
        limit: 20,
      })
      .extract({
        username: "get username of this profile",
        repos:
          "repos they commit to. for repo, ONLY include the the last part of th repo, with no slash",
      })
